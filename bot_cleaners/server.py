...
from .model import Habitacion, RobotLimpieza, Celda, Mueble, Estacion, get_movimientos, get_estacion_cargas
...
def agent_portrayal(agent):
    if isinstance(agent, RobotLimpieza):
        ...
        return {"Shape": "rect", "Filled": "true", "Color": "#FBD172", "Layer": 0,
                "w": 0.9, "h": 0.9}
    elif isinstance(agent, Estacion):
        return {"Shape": "rect", "Filled": "true", "Color": "Gray", "Layer": 0, "text": "⚡",
                "w": 0.9, "h": 0.9}
...
chart_celdas = mesa.visualization.ChartModule(
    [{"Label": "CeldasSucias", "Color": '#36A2EB', "label": "Celdas Sucias (Porcentaje)"}],
    100, 200,
    data_collector_name="datacollector"
)
num_movs = mesa.visualization.ChartModule(
    [{"Label": "Movimientos", "Color": '#C21807', "label": "Movimientos Totales"}],
    100, 200,
    data_collector_name="datacollector"
)
num_cargas = mesa.visualization.ChartModule(
    [{"Label": "Cargas totales", "Color": '#9A9A9A', "label": "Cargas en Estacion Totales"}],
    100, 200,
    data_collector_name="datacollector"
)
...
server = mesa.visualization.ModularServer(
    Habitacion, [grid, chart_celdas, num_movs, num_cargas],
    "Robots Limpiadores", model_params, 8521
)
